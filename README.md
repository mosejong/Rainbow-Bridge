...
| ② | 보호자 감정 체크인 | 오늘의 감정 상태 기록 + 위기 감지 | ✅ |
| ③ | AI 추모 메시지 생성 | Gemini AI 개인화 위로 메시지 (RAG 기반) | ✅ |
| ④ | TTS 음성 낭독 | warm·calm·hopeful 톤 선택 | 🟡 엔진 확정 중 |
| ⑤ | 일상 복귀 미션 추천 | 회복 단계별 행동 미션 (AI+RAG) | ✅ |
| ⑥ | 추모 타임라인 | 감정·메시지·미션 기록 시간순 보관 | ✅ |
| ⑦ | 위기 감정 안전 라우팅 | risk_level L0~L3, L2↑ → 1393 즉시 안내 | ✅ |
| ⑧ | 회복 평가 리포트 | 감정 추이·미션 완료율 시각화 | 🟡 |

**단계별 케어 모듈(AI):** 증상진료 안내 ✅ · 장례절차 상담 ✅ · 기념일 케어(D+30·D+100) ✅

**멀티모달(가산):** 사진 → LivePortrait → 말하는 동물 영상 → TTS 합성 → 다운로드 ✅

> 진행 상황: [docs/PROGRESS.md](docs/PROGRESS.md) · 기능 정의서: [docs/FEATURE_SPEC.md](docs/FEATURE_SPEC.md)
...
| 영역 | 기술 |
|------|------|
| Backend | FastAPI (Python), MongoDB, PostgreSQL, Redis |
| Frontend | React Native + Expo SDK 52 (Android/iOS) |
| AI / LLM | Gemini API (`gemini-2.5-flash`) |
| RAG | ChromaDB (comfort · mission · funeral · vet_protocol) |
| TTS | Google Cloud TTS + 감정 튜닝 (LLM 히든워드) |
...
│   └── evaluation/   # 평가 리포트
└── docs/             # 📚 문서
    ├── FEATURE_SPEC.md   # 기능 정의서 (현행)
    ├── SERVICE_FRAME.md  # 서비스 범위·기능 틀 (왁구)
