"""④ TTS 합성 — Qwen3-TTS VoiceDesign 연동 (어린아이 확정 보이스).

tts.py(Google Cloud TTS)와 **같은 계약** `synthesize(text, tone)`을 Qwen3 로컬
GPU 엔진으로 제공합니다. 보이스는 정환주가 다이얼에서 고정한 **두 종만** 노출:
  - "boy"  = Downloads/audio (10).wav  (PROTOTYPE_VOICE.md #3)
  - "girl" = Downloads/audio (6).wav   (PROTOTYPE_VOICE.md #4)
새 텍스트를 이 두 목소리(seed 고정)로 낭독합니다. 임의 화자 생성 아님.
...
# 정환주 확정 보이스 2종 (PROTOTYPE_VOICE.md #3/#4 와 동기화 — seed 가 화자 ID).
# 값 바꾸면 다른 목소리가 됨. 특히 seed 는 고정 필수.
_VOICES: dict[str, dict] = {
    # audio (10).wav — boy, 차분, 밝기 살짝
    "boy": dict(
        gender="boy",
        age="older",
        warmth=1,
