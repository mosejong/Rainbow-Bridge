...
## #5 — woman 차분 (성인 여성, 프로토타입 OK ✅)

- **원본 파일**: `ai/tts/_output/qwen3_emotion/qwen3em_dial_woman_child_w1_b1_low_calm_c1_slow_t0.5_s21424.wav`
- 사용자 다운로드본: `Downloads/audio (12).wav` (동일 오디오 — 파형 diff 0.000076 확인)
...
|------|-----|
| 성별 | **woman** (성인 여성) |
| 나이 | child (성인은 **무시**됨 — build_instruct가 성인 앵커 사용) |
| 다정함 | 1 |
| 밝기 | 1 |
| 피치 | **low** |
| 감정 결 | **calm** (차분) |
...
> 성인 여성 보이스(어린이 #1~#4와 별개). build_instruct에 man/woman 성인 앵커 분기를 추가해 생성.
> ⚠️ **속도 주의**: 다이얼 `slow`인데도 말이 빨라서, 실사용 시 `atempo=0.9` 후처리로 늦춤
> (→ [오디오_후처리_가이드.md](오디오_후처리_가이드.md) §1). 후처리본: `Downloads/audio12_slow90.wav`.
