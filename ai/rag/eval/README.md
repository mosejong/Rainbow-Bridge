...
> 작성: 정환주 · 2026-06-05 · RAG 배관이 **실제로 의미검색이 되는지** 실 임베딩으로 측정.
...
→ 패러프레이즈 쿼리 12개 전부 정답 토픽을 **rank 1**로 끌어옴(유사도 0.73~0.83). ChromaDB + Gemini 임베딩 배관이 의미검색으로 정상 동작함을 확인.

## 실행

```bash
cd ai
python -m rag.eval.eval_retrieval        # top-3
