...
### 향후 설계 B — driving 영상 자동 선택 (회의 논의용, 모세종님 제안)

> 현재는 driving을 고정값으로 사용. **입력 사진 특징에 따라 최적 driving을 자동 선택**하면 품질 균일화 가능.

**동작 흐름(안)**
```
소스 사진 입력
 → 얼굴 특징 추출(입 열림/닫힘·포즈·눈 크기 등)
 → DB에서 매칭 driving 조회
 → 해당 driving으로 합성
```

**이미 보이는 매칭 패턴(부록 A 데이터)**
- 입 다문 사진 → 입 움직임 잘 보임 / 입 벌린 사진 → 묻힘
- 말티즈(입 벌림) 최적: EdMarkey0 · 고양이(입 다뭄) 최적: d11

**특징 추출 방식 비교**
| 방식 | 속도 | 동물 정확도 | 추가 설치 |
|------|------|:---:|------|
| **XPose (LivePortrait 내장)** | 빠름 | ★★★★★ | 없음 ✅ |
| MediaPipe | 매우 빠름 | ★★☆☆☆ | pip 1개 |
| dlib | 보통 | ★★☆☆☆ | 번거로움 |
...
- 매칭 실패(미등록 종) 시 폴백 driving = d11(단일 클립·범용 양호)
- 구현 범위: 우선 "입 열림/닫힘" 2분기부터 → 점진 확장
- 자동 선택 vs **사진 입력 가이드**(입 다문 정면 권장) 중 어디까지 자동화할지
