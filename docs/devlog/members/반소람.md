...
**기능 (내 작업) — 우선순위 순**
- [ ] **L1/L2/L3 차등 응답 라우팅 + 복지자원 안내** — 현재 "L2↑→1393 / 그 외→평소 생성" 이분법을 L1(공감 강화)·L2(1393)·L3(생성중단+전면) 분기 + 복지자원 경로로 (中)
- [ ] **질병 triage RAG 신규 구축** — `vet_protocol` 코퍼스 작성 + `triage.py` RAG 배선(현재 미사용) + 프롬프트 반영 + 검증 스크립트 (大)
...
**팀 협의 (급하지 않음 — 시간 될 때)**
- [ ] 모세종: recovery_trend 생성 경로 통합 결정 (백엔드 `message.py`에 병렬 구현 존재)
- [ ] 모세종: 질병 triage 기능 범위("기본 처치 안내 + 내원 유도")·"의료 조언 아님" 면책 합의
