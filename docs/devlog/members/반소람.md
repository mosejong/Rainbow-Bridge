...
### 한 일
- [x] **⑤ 미션 추천 로직** — `mission.py` `recommend(emotion_score, day_since, history, *, generate)`
  - 감정 점수 → 난이도(gentle/small/active) **규칙 매핑** (무기력엔 아주 작은 미션부터)
  - **LLM(Gemini) 개인화** + 실패/이상 출력 시 **규칙 풀 폴백**(graceful) — memorial 과 같은 패턴
  - `history` 중복 회피, 부활 표현 가드, `json_mode` 출력
- [x] **`prompts/mission.py`** — 미션 프롬프트(경계: 작게·안전·보호자 대상, 위험 미션 금지)
- [x] **`tests/test_mission.py`** 10종(난이도·폴백·history·가드) — 전체 **115종 통과**
- [x] **⑦ LLM 분류 레이어(L1)** — `safety.py` `classify_with_llm()` + `assess_crisis()` 보수적 융합
  - 규칙(L0)이 놓치는 완곡·맥락 표현을 LLM(Gemini)으로 보완
  - 융합 = `max(L0, L1)` → 등급 절대 안 내림(미탐 0 보장)
  - `generate` 미주입 시 규칙만 반환(백엔드 기존 호출 `assess_crisis(note)` 그대로 동작)
  - LLM 실패·JSON 깨짐 → 규칙 결과로 graceful 폴백
- [x] **`tests/test_safety_llm.py`** 신규 10종(융합·폴백·파싱·클램프) — 전체 **125종 통과**
- [x] **`validate_crisis_llm.py`** — 실제 Gemini로 골든셋 수동 검증 도구 (CI 미포함)

### 결정 사항
- 미션 추천 = **규칙(난이도·안전) + LLM(문구 개인화) + 폴백** 하이브리드
- L1 활성화는 팀 결정 사항 — 백엔드가 `assess_crisis(note, generate=provider.generate)` 로 호출하면 켜짐 (비용·레이트리밋 발생)
- 입력 키 `emotion_score`·`day_since` 백엔드 `MissionRecommendRequest` 와 일치

### 막힌 것 / 고민
- Gemini 무료 티어 **분당 요청 제한** → 연속 호출 시 폴백 발동(서비스엔 무영향). 시연 多호출 시 로컬 Ollama/PERSO 고려
- 출력 `[{title, description, category}]` vs 백엔드 `missions: list[str]` → 제목만 쓸지/스키마 확장할지 가벼운 협의 필요

### 의미
- **반소람 담당 AI 5종 완료**: ⑦ 위기 감지(L0+L1) · provider(Gemini) · ③ 추모 메시지 · ⑤ 미션 추천
