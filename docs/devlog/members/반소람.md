...
## 2026-06-04 — ⑤ 미션 추천 로직 구현 (마지막 AI 조각)
...
- [x] 실제 Gemini 라이브 검증: 감정별 난이도 조절·개인화·폴백 동작 확인
- [x] dev 머지(충돌 없음) → 백엔드 `detect_crisis` 실연동(#38)·ai 패키지 경로 반영 확인
...
### 막힌 것 / 고민
- Gemini 무료 티어 **분당 요청 제한** → 연속 호출 시 폴백 발동(서비스엔 무영향, 폴백이 흡수). 시연 多호출 시 로컬 Ollama/PERSO 고려
- 출력 `[{title, description, category}]` vs 백엔드 `missions: list[str]` → 제목만 쓸지/스키마 확장할지 가벼운 협의 필요

### 의미
- **반소람 담당 AI 핵심 4종 완료**: ⑦ 위기 감지 · provider(Gemini) · ③ 추모 메시지 · ⑤ 미션 추천
