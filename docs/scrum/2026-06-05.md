...
### 시스템 구성도
![시스템 구성도](../system_architecture_final.svg)

### 사용자 흐름도
![사용자 흐름도](../user_flow_fixed.svg)
...
| RAG 공통 필수 | ✅ | ChromaDB RAG 파이프라인 구축 완료 (정환주, PR #79) |
...
| AI | 정환주 | ChromaDB RAG 파이프라인 전체 구현 (embeddings·store·ingest·retrieve), Gemini 임베딩 실검증 | ✅ |
...
2. **RAG 코퍼스 범위**
   - 파이프라인(ChromaDB) 완성, 실제 위로글 코퍼스는 미작성
   - 샘플 5개로 데모 가능한지, 실 데이터 필요한지 확인

3. ~~**PERSO 립싱크 발표 전략 동의 여부**~~ ✅ 6/4 강사님 승인
   - "기술 가능 → 윤리적 이유로 제한" 서사 확정

---

## 📅 다음 스크럼까지 목표

- [x] 반소람: 실 위로글 코퍼스 작성 → RAG ingest
